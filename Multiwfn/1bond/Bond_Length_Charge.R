...
data=dir(path="F:/2ndALL/0 Gaussian16/HF_N_fchk/ADCH Charges/", pattern="\\.chg")
...
for (i in 1:length(chemical)){
  ...
  fpchg <- paste("F:/2ndALL/0 Gaussian16/HF_N_fchk/ADCH Charges/p", chemical[i], ".chg", sep="")
  fptxt <- paste("F:/2ndALL/0 Gaussian16/HF_N_fchk/Bond_Length/p", chemical[i], "_Bond_Length_Chargenouse.txt", sep="")  
  ...
  for (j in C){
	...
	df$QCNegativemax[rindx] = max(QcNegative)
...
write.csv(df, file="F:/2ndALL/0 Gaussian16/Chg_txt_bond_length_charge_173_to.csv", row.names=F)
